...
module Test.HAPI.ApiTrace where
...
data ApiTraceEntry (api :: ApiDefinition) where
  TraceCall :: All Fuzzable p => PKey a -> api p a -> NP DirectAttribute p -> ApiTraceEntry api
...
traceCall :: All Fuzzable p => PKey a ->  api p a -> NP DirectAttribute p -> ApiTrace api
