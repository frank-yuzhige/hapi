...
The implementation is similar to that shown in the paper.
 "Embedding effect systems in Haskell" Orchard, Petricek 2014
  Slightly modified on top of https://hackage.haskell.org/package/type-level-sets-0.8.9.0/docs/src/Data.Type.Map.html
 since the original package does not use Nat. -}

{-# LANGUAGE TypeOperators, PolyKinds, DataKinds, TypeFamilies, UndecidableInstances,
             MultiParamTypeClasses, FlexibleInstances, GADTs, FlexibleContexts, ScopedTypeVariables,
             ConstraintKinds #-}




module Test.HAPI.Data.NatMap where
import GHC.TypeLits
...
{-| Pair a Nat (representing a variable) with a type -}
data Var (k :: Nat) = Var

instance KnownNat k => Show (Var k) where
    show = show . natVal

{-| A value-level heterogenously-typed Map (with type-level representation in terms of lists) -}
data Map (n :: [Mapping Nat *]) where
    Empty :: Map '[]
...
instance (KnownNat k, Show v, Show' (Map s)) => Show (Map ((k :-> v) ': s)) where
...
type instance Cmp (k :: Nat) (k' :: Nat) = CmpNat k k'
type instance Cmp (k :-> v) (k' :-> v') = CmpNat k k'
